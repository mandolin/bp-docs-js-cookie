...
本地/CI 文档工程矩阵：直接 JPHS/JTH、hia-jsdoc 独立输出与 Portal bridge、统一 Portal 三条输出链，共 27 个配置 profile、
36 个可浏览 surface；公开站点则只发布默认 `unified-portal.fetch.classic` 产品入口。建议从
...
local/CI matrix spans direct JPHS/JTH, hia-jsdoc standalone plus Portal bridge, and unified Portal pipelines: 27 configuration profiles
and 36 browsable surfaces. The public site publishes only the default `unified-portal.fetch.classic` product entry. Start with
[`docs/hia/README.md`](docs/hia/README.md), then use the architecture and Cookie-boundary guides above.
...
All local/CI profiles are multi-page and can switch among three owner-provided skins. `fetch` is the default source-reading mode; `embed` and
`link` are explicit alternatives, while `none` remains a separate privacy regression. Fetch uses same-origin, content-addressed,
digest-verified text assets and loads only on expansion. The workflow requires byte-identical local/CI and public artifacts from Node
...
owners and contains only the sanitized default Portal/fetch/classic product. The 27-profile chooser remains a local/CI test surface and
is not uploaded to Pages.
